import 'bootstrap/dist/css/bootstrap.min.css';
import { Navbar } from 'react-bootstrap';
import { Link } from 'react-router-dom';
import { LogoutButton} from './AuthComponents';

function NavigationBar (props) {
    return (
        <Navbar className="myNav" expand="sm">
          <Link to="/">
            <NavBrand></NavBrand>
          </Link>
            <NavTitle></NavTitle>
            <Link to="/login">
              {!props.loggedIn && <NavUser></NavUser>}
            </Link>
            {props.loggedIn && <LogoutButton logout={props.handleLogout} />}
        </Navbar>
    );
}

function NavBrand () {
    return (
        <i className="bi bi-mortarboard-fill myLogo"></i>
    );
}

function NavTitle(){
  return(
    <h2 className="myTitle">STUDY PLAN</h2> //missing css
  )
}

function NavUser () {
    return (
        <div className="navbar-nav ms-md-auto">
              <svg className="bi bi-people-circle myUser" width="35" height="35" viewBox="0 0 16 16" fill="white" xmlns="http://www.w3.org/2000/svg">
                  <path d="M13.468 12.37C12.758 11.226 11.195 10 8 10s-4.757 1.225-5.468 2.37A6.987 6.987 0 008 15a6.987 6.987 0 005.468-2.63z"/>
                  <path fillRule="evenodd" d="M8 9a3 3 0 100-6 3 3 0 000 6z" clipRule="evenodd"/>
                  <path fillRule="evenodd" d="M8 1a7 7 0 100 14A7 7 0 008 1zM0 8a8 8 0 1116 0A8 8 0 010 8z" clipRule="evenodd"/>
              </svg>
        </div>
    );
}

export {NavigationBar};
